...
export async function sendInviteEmail(email: string, name: string) {
    ...
    const mailOptions = {
        from: '"Momentum Community" <' + process.env.EMAIL_USER + '>',
        to: email,
        subject: "Welcome to Momentum - You're In!",
        html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #333; border-radius: 10px; background-color: #000; color: #fff;">
        <h1 style="color: #fff; font-size: 24px; margin-bottom: 20px;">Welcome to Momentum, ${name}!</h1>
        ...
        <div style="margin: 30px 0; text-align: center;">
          <a href="${whatsappLink}" style="background-color: #ea580c; color: #fff; padding: 15px 30px; text-decoration: none; border-radius: 50px; font-weight: bold; font-size: 16px; display: inline-block;">
            Join the WhatsApp Community
          </a>
        </div>

        <p style="color: #888; font-size: 14px; margin-top: 30px; text-align: center;">
          Note: This invite link is unique to you. Please do not share it.
        </p>
